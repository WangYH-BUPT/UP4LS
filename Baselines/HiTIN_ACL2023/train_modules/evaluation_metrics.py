...
def evaluate(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    ...
    # test_acc = accuracy_score(epoch_predicts, test_label)
    # test_weighted_f1 = f1_score(test_pred, test_label, average='binary')

    # epoch_predicts = [p[0] for p in epoch_predicts]
    # test_acc = accuracy_score(epoch_predicts, epoch_labels)
    # precision = precision_score(epoch_predicts, epoch_labels)
    # recall = recall_score(epoch_predicts, epoch_labels)
    # test_f1 = f1_score(epoch_predicts, epoch_labels, average='binary')
    # print(test_acc)
    # print(precision)
    # print(recall)
    # print(test_f1)
    ...
    for i, label in id2label.items():
        precision_dict[label], recall_dict[label], fscore_dict[label] = _precision_recall_f1(right_count_list[i],
                                                                                             ...
                                                                                             gold_count_list[i])
        # print("i",i)
        # print(right_count_list[i])
        # print(predicted_count_list[i])
        # print(gold_count_list[i])
    ...
    # acc = float(right_total) / (predict_total)
    # print("right_count_list", right_count_list)
    # print("predict_total", predicted_count_list)
    # print("gold_total", gold_count_list)
    ...
    acc = float(right_total) / predict_total if predict_total>0 else 0.0
    precision = float(right_count_list[0]) / predicted_count_list[0] if predicted_count_list[0]>0 else 0.0
    recall = float(right_count_list[0]) / (right_count_list[0]+ predicted_count_list[1]-right_count_list[1]) if (right_count_list[0]+ predicted_count_list[1]-right_count_list[1])>0 else 0.0
