...
def train(config, args):
    ...
    if not os.path.isdir(model_checkpoint):
        ...
        if os.path.isfile(os.path.join(model_checkpoint, latest_model_file)):
            ...
            logger.info('Previous Best Performance---- f1: {}%'.format(
                best_performance[0]))
    ...
    for epoch in range(config.train.start_epoch, config.train.end_epoch):
        ...
        print("[Test] epoch: %d, performance↓\n precision: %.4f\t recall: %.4f\tf1: %.4f\nacc: %.4f" \
        ...
        if not (performance['f1'] >= best_performance[0] ):
            wait += 1
        ...
        if performance['f1'] > best_performance[0]:
            wait = 0
            logger.info('Improve f1{}% --> {}%'.format(best_performance[0], performance['f1']))
            best_performance[0] = performance['f1']
            best_epoch[0] = epoch
            save_checkpoint({
                'epoch': epoch,
                'model_type': config.model.type,
    ...
    if os.path.isfile(best_epoch_model_file):
        ...
        #             % (best_epoch[0], performance['micro_f1'], performance['macro_f1']))
        print("Best micro-f1 on epoch: %d, [Test] performance↓\n f1: %.4f\nacc: %.4f" \
                    % (best_epoch[0], performance['f1'], performance['acc']))
