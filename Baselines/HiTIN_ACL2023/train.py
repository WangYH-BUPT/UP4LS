...
def train(config, args):
    ...
    for epoch in range(config.train.start_epoch, config.train.end_epoch):
        ...
        print("[Val] epoch: %d precision: %.4f\t recall: %.4f\t f1: %.4f\t acc: %.4f" \
                    % (epoch, performance['precision'], performance['recall'], performance['f1'], performance['acc']))
        ...
        if performance['f1'] > best_performance:
            wait = 0
            logger.info('Improve f1 %.4f --> %.4f' %(best_performance, performance['f1']))
            ...
            }, os.path.join(model_checkpoint, 'best_f1_' + model_name))

        performanceTest = trainer.eval(test_loader, epoch, 'TEST')
        ...
        print("[Test] epoch: %d, performance | precision: %.4f\t recall: %.4f\tf1: %.4f\t acc: %.4f" \
              % (epoch, performanceTest['precision'], performanceTest['recall'], performanceTest['f1'], performance['acc']))

        # saving best model and check model
        # if not (performance['micro_f1'] >= best_performance[0] or performance['macro_f1'] >= best_performance[1]):

        if performanceTest['f1']> best_Test_f1:
            best_Test_f1 = performanceTest['f1']
            best_Test_acc = performanceTest['acc']
    ...
    print("[Best] test: performance | acc:%.4f  |  f1:%.4f ." %(best_Test_f1, best_Test_acc))
    logger.info("[Best] test: performance | acc:%.4f  |  f1:%.4f ." %(best_Test_f1, best_Test_acc))

    # best_epoch_model_file = os.path.join(model_checkpoint, 'best_f1_' + model_name)
