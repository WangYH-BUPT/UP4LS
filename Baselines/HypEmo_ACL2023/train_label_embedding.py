...
parser.add_argument("--tree", type=str, help='go_emotion | ED | ED_easy_4 | ED_hard_a | ED_hard_b | ED_hard_c | ED_hard_d | mydata', default='mydata')
...
default_params = {
    ...
    'tree_depth' : 2, # a number between 1 and 7
    'level_branch' : 2,  # a number between 3 and 4
    'remove_root': True,
    # 'remove_root': False,
    ...
    'epsilon': 1e-5,
    'seed': 7, # random seed
    'num_negative': 2, # Number of negative samples to use (5 for go_emotion, 10 for empathatic dialogs)
...
if args.tree == 'go_emotion':
    ...
    data_file_path = os.path.join(data_directory, 'label_tree.tsv')
    root_label = 'root'
elif args.tree == 'mydata':
    data_directory = os.path.join((os.path.abspath('')), 'data', 'mydata')
    data_file_path = os.path.join(data_directory, 'label_tree.tsv')
    root_label = 'root'
...
print(len(transitive_relations))
print(len(tree_relations))
