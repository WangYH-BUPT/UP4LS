...
all_dataset_list = ['go_emotion', 'ED', 'ED_easy_4', 'ED_hard_a', 'ED_hard_b', 'ED_hard_c', 'ED_hard_d', 'mydata']
ENCODER_TYPE = 'roberta-base'
temperature = 0.3

config_args = {
    'training_config': {
        'batch_size': (100, 'batch size for training'), # 100
        'epochs': (20, 'maximum number of epochs to train for'), # 20
    ...
    'data_config': {
        'dataset': ('mydata', 'which dataset to use'),
    }
}
...
def get_dicts(dataset, return_emb_dicts=False):
    ...
    # elif dataset == 'mydata':
    #     label2idx = {'cover1': 0, 'cover2':1, 'stego1':2, 'stego2':3}
    elif dataset == 'mydata':
        label2idx = {'cover': 0, 'stego':1}
